...
public static partial class AgcExports
{
    ...
    [SysAbiExport(
        ...
        Target = Generation.Gen5,
        LibraryName = "libSceAgc")]
    public static int DcbSetCxRegisterDirectGetSize(CpuContext ctx) =>
        DcbSetRegisterDirectGetSize(ctx);
    ...
    public static int DmaDataPatchSetSrcAddressOrOffsetOrImmediate(CpuContext ctx)
    {
        ...
        return ctx.TryWriteUInt64(commandAddress + 24, ctx[CpuRegister.Rsi])
            ? SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_OK)
            : SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT);
    }
    ...
    public static int WaitRegMemPatchAddress(CpuContext ctx)
    {
        ...
        if (TryGetPacketIdentity(ctx, commandAddress, out var packetOp, out var register) &&
            packetOp == ItNop &&
            register is RWaitMem32 or RWaitMem64)
        {
            return TryWriteUInt32(
                       ctx,
                       commandAddress + 4,
        ...
        var fieldOffset = op == ItWaitRegMem ? 8UL : 4UL;
    ...
    public static int QueueEndOfPipeActionPatchAddress(CpuContext ctx)
    {
        ...
        if (!TryReadUInt32(ctx, commandAddress + 12, out var oldLow))
        {
            return SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT);
        }

        return TryWriteUInt32(ctx, commandAddress + 12, (oldLow & 3u) | ((uint)address & ~3u)) &&
               TryWriteUInt32(ctx, commandAddress + 16, (uint)(address >> 32))
            ? SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_OK)
            : SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT);
    }
    ...
    {
        const int resourceRegistrationUnavailable = unchecked((int)0x8A6C9018);
        ctx[CpuRegister.Rax] = unchecked((ulong)resourceRegistrationUnavailable);
        return resourceRegistrationUnavailable;
    }
        ...
        bool tracePackets)
    {
        ...
        while (offset < dwordCount)
        {
            ...
            if ((op == ItAcquireMem || (op == ItNop && register == RAcquireMem)) && length >= 8)
            {
        ...
        bool indexed)
    {
        ...
        // These modes are fixed-function colour-buffer metadata operations.
        // Translating their bound shader as an ordinary colour draw overwrites
        // the target that later passes expect to sample.
        ...
        out string error)
    {
        ...
        if (!Gen5ShaderTranslator.TryCreateState(
            ...
            !Gen5ShaderScalarEvaluator.TryEvaluate(
                ...
                requiredVertexRecordCount: TryAttachNggComputeCapture(
                    ctx,
                    state,
                    vertexCount,
                    indexed,
                    countIsExact: !indexed && state.InstanceCount > vertexCount)))
        {
            return false;
        }
    ...
    private static uint? TryAttachNggComputeCapture(
        CpuContext ctx,
        SubmittedDcbState state,
        ...
        bool countIsExact)
    {
        // A flattened instanced NGG draw has an authoritative invocation
        // count. NumRecords describes fetch-buffer capacity and must not cap
        // the capture range in that case.
        if (countIsExact)
        {
            return Math.Max(invocationCount, Math.Max(state.InstanceCount, 1u));
        }

        return TryGetRequiredVertexRecordCount(
            ctx,
            state,
            invocationCount,
            indexed,
        ...
        out ulong commandAddress)
    {
        ...
        var availableDwords = cursorDown >= cursorUp
            ...
            : 0;
        var remainingDwords = availableDwords > reservedDwords
            ? availableDwords - reservedDwords
            : 0;
        ...
        uint pollCycles)
    {
        ...
        return TryWriteUInt32(ctx, commandAddress, Pm4(8, ItAcquireMem, 0)) &&
               TryWriteUInt32(ctx, commandAddress + 4, 0) &&
               TryWriteUInt32(ctx, commandAddress + 8, 0) &&
               TryWriteUInt32(ctx, commandAddress + 12, 0) &&
               TryWriteUInt32(ctx, commandAddress + 16, 0) &&
               TryWriteUInt32(ctx, commandAddress + 20, 0) &&
               TryWriteUInt32(ctx, commandAddress + 24, pollInterval) &&
               TryWriteUInt32(ctx, commandAddress + 28, 0x100);
    }
        ...
        uint pollCycles)
    {
        ...
        const ulong maximumAddress = 0x0000_FFFF_FFFF_FF00UL;
        var maximumRange = baseAddress < maximumAddress ? maximumAddress - baseAddress : 0;
        ...
        return TryWriteUInt32(ctx, commandAddress, Pm4(8, ItAcquireMem, 0)) &&
               ...
               TryWriteUInt32(ctx, commandAddress + 8, (uint)(range >> 8)) &&
               TryWriteUInt32(ctx, commandAddress + 12, (byte)(range >> 40)) &&
               TryWriteUInt32(ctx, commandAddress + 16, (uint)(baseAddress >> 8)) &&
               TryWriteUInt32(ctx, commandAddress + 20, (uint)(baseAddress >> 40)) &&
               TryWriteUInt32(ctx, commandAddress + 24, pollInterval) &&
               TryWriteUInt32(ctx, commandAddress + 28, gcrControl & 0x7_FFFFu);
    }
    ...
    private static int DcbSetRegisterDirectGetSize(CpuContext ctx)
    {
        ctx[CpuRegister.Rax] = 3u * sizeof(uint);
        return (int)ctx[CpuRegister.Rax];
    }
