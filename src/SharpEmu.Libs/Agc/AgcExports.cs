...
public static class AgcExports
{
        ...
        bool paddedTiledRead)
    {
        ...
        // Some guests build their surfaces in libc-heap allocations that are
        // GPU-aliased rather than in the emulated GPU address space; fall back
        // to the tracked-heap alias reader before giving up.
        if (KernelMemoryCompatExports.TryReadTrackedLibcHeapGpuAlias(
                descriptor.Address,
                destination))
        {
            return true;
        ...
        var unpaddedByteCount = GetTextureByteCount(
            ...
            descriptor.Width,
            descriptor.Height);
        if (unpaddedByteCount == 0 || unpaddedByteCount >= (ulong)destination.Length)
        {
            return false;
        }

        var unpadded = destination.AsSpan(0, (int)unpaddedByteCount);
        return ctx.Memory.TryRead(descriptor.Address, unpadded) ||
