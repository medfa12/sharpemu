...
public sealed partial class DirectExecutionBackend
{
	...
	private static readonly bool _perfHleHistogram =
		...
		string.Equals(System.Environment.GetEnvironmentVariable("SHARPEMU_LOG_HLE_HIST"), "1", System.StringComparison.Ordinal);
	...
	private volatile bool _ripSamplerStop;
	private int _ripSamplerRunning;
	...
	private void DumpRuntimeCode()
	{
			...
			StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
		{
			...
			var baseText = parts[0].StartsWith("0x", StringComparison.OrdinalIgnoreCase)
				? parts[0][2..]
				: parts[0];
			if (!ulong.TryParse(
					baseText,
			...
			var chaseSucceeded = true;
			for (var index = 1; index < parts.Length; index++)
			{
				var offsetText = parts[index].StartsWith("0x", StringComparison.OrdinalIgnoreCase)
					? parts[index][2..]
					: parts[index];
				if (!ulong.TryParse(
						offsetText,
						...
						System.Globalization.CultureInfo.InvariantCulture,
						out var offset) ||
					!memory.TryRead(address, pointerBytes))
				{
					Console.Error.WriteLine(
						$"[LOADER][CODEDUMP] chase '{token}' unreadable at 0x{address:X16}");
					chaseSucceeded = false;
					break;
				}

				address =
					System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(pointerBytes) +
					offset;
			}
			...
			{
				Console.Error.WriteLine(
					$"[LOADER][CODEDUMP] addr=0x{address:X16} bytes={Convert.ToHexString(bytes)}");
			}
			else
			{
				Console.Error.WriteLine(
					$"[LOADER][CODEDUMP] addr=0x{address:X16} unreadable");
			}
		}
	}
	...
	{
		if (Interlocked.CompareExchange(ref _ripSamplerRunning, 1, 0) != 0)
		{
			return;
		}

		_ripSamplerStop = false;
		var sampler = new Thread(() =>
		{
			try
			{
				...
				DumpRuntimeCode();
				var ripHistogram = new Dictionary<ulong, long>();
				var callHistogram = new Dictionary<ulong, long>();
				var stopwatch = System.Diagnostics.Stopwatch.StartNew();
				long nextDumpMilliseconds = 5000;
				const ulong guestCodeStart = 0x800000000UL;
				const ulong guestCodeEnd = 0x810000000UL;
				var stackBytes = new byte[96 * sizeof(ulong)];
				var codeBytes = new byte[64];

				while (!_ripSamplerStop && !_forcedGuestExit)
				{
					try
					{
						foreach (var thread in SnapshotGuestThreads())
						{
							var rip = thread.Context.Rip;
							var rsp = thread.Context[CpuRegister.Rsp];
							...
							if (rip < guestCodeStart || rip >= 0x900000000UL)
							{
								continue;
							}

							ripHistogram.TryGetValue(rip, out var ripCount);
							ripHistogram[rip] = ripCount + 1;
							if (rsp == 0 || !thread.Context.Memory.TryRead(rsp, stackBytes))
							{
								continue;
							}
							...
							var seen = 0;
							for (var offset = 0;
								offset + sizeof(ulong) <= stackBytes.Length && seen < 12;
								offset += sizeof(ulong))
							{
								var candidate =
									System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(
										stackBytes.AsSpan(offset, sizeof(ulong)));
								if (candidate < guestCodeStart || candidate >= guestCodeEnd)
								{
									continue;
								}

								callHistogram.TryGetValue(candidate, out var callCount);
								callHistogram[candidate] = callCount + 1;
								seen++;
							}
						}
					}
					...
					{
						// Guest threads can exit between the snapshot and memory read.
					}

					Thread.Sleep(15);
					if (stopwatch.ElapsedMilliseconds < nextDumpMilliseconds)
					{
						continue;
					}

					nextDumpMilliseconds += 5000;
					var topRips = ripHistogram
						.OrderByDescending(pair => pair.Value)
						.Take(15)
						.ToArray();
					Console.Error.WriteLine(
						"[LOADER][RIPSAMPLE] guest-rip top15: " +
						string.Join(
							" | ",
							topRips.Select(pair => $"0x{pair.Key:X10}={pair.Value}")));
					var topCallSites = callHistogram
						.OrderByDescending(pair => pair.Value)
						.Take(25)
						.ToArray();
					Console.Error.WriteLine(
						"[LOADER][CALLSITE] guest call-sites top25: " +
						string.Join(
							" | ",
							topCallSites.Select(pair => $"0x{pair.Key:X10}={pair.Value}")));
					...
					foreach (var callSite in topCallSites.Take(8))
					{
						var start = callSite.Key >= 48 ? callSite.Key - 48 : callSite.Key;
						if (memory.TryRead(start, codeBytes))
						{
							Console.Error.WriteLine(
								$"[LOADER][CALLBYTES] site=0x{callSite.Key:X10} " +
								$"start=0x{start:X10} bytes={Convert.ToHexString(codeBytes)}");
						}
					}
				}
			}
			...
			{
				Volatile.Write(ref _ripSamplerRunning, 0);
			}
		})
		{
			IsBackground = true,
			Name = "SharpEmu RIP sampler",
		};
		sampler.Start();
	}
